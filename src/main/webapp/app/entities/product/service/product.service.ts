...
export class ProductService {
  ...
  protected convertDateFromClient(product: IProduct): IProduct {
    return Object.assign({}, product, {
      expire: product.expire?.isValid() ? product.expire.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.expire = res.body.expire ? dayjs(res.body.expire) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((product: IProduct) => {
        product.expire = product.expire ? dayjs(product.expire) : undefined;
      });
    }
    return res;
  }
