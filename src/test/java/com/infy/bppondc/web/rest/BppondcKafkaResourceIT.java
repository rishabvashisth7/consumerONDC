...
import com.infy.bppondc.config.EmbeddedKafka;
import com.infy.bppondc.config.KafkaSseConsumer;
import com.infy.bppondc.config.KafkaSseProducer;
...
class BppondcKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bppondc-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/bppondc-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/bppondc-kafka/unregister"));
                return;
            }
        }
